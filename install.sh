...
create_env_file() {
    ...
    # Create .env file (with comments, since dotenv handles them)
    cat > "${JELLYSTAT_DIR}/backend/.env" << EOF
...
EOF
    ...
    # Secure the .env file
    chmod 600 "${JELLYSTAT_DIR}/backend/.env"
    chown "${JELLYSTAT_USER}:${JELLYSTAT_GROUP}" "${JELLYSTAT_DIR}/backend/.env"

    log_info "Environment file created at ${JELLYSTAT_DIR}/backend/.env"
}
